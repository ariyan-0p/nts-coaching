...
import {
  ...
  TrendingUp, BarChart2, PieChart, Layers, Phone, Mail
} from 'lucide-react'
    ...
    <div className="page-enter">
      ...
      <section className="fmp-hero">
        ...
        <div className="container" style={{ position: 'relative', zIndex: 1 }}>
          <Link to="/" className="fmp-back-link"
          ...
          <div className="fmp-hero-grid">
            <motion.div initial={{ opacity: 0, x: -30 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.7 }}>
              ...
              <h1 className="fmp-h1">Financial <span style={{ color: 'var(--red)' }}>Market</span><br />Professional</h1>
              ...
              <p style={{ fontSize: '1rem', color: 'var(--text-muted)', lineHeight: 1.8, fontWeight: 300, marginBottom: 32, maxWidth: 480 }}>
                A comprehensive 4-module certified program covering Stock Market Fundamentals, Money Management, Technical Analysis, and Derivatives — designed for students and professionals who wish to master the stock market and achieve financial freedom.
              </p>
              ...
              <div style={{ background: 'var(--bg-2)', border: '1px solid var(--border)', borderTop: '3px solid var(--red)', padding: '32px 28px' }}>
                ...
                <div style={{ marginTop: 28, padding: '20px', background: 'var(--red-dim)', border: '1px solid var(--border-red)' }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 4, marginBottom: 16 }}>
                    {[['Duration', '2.5 Months'], ['Batch Timing', '8:30 PM – 9:30 PM'], ['Schedule', '3 days/week (MTT / WFS)']].map(([k, v]) => (
            ...
            {modules.map((mod, i) => (
              ...
              >
                <div className="fmp-module-label">
                  ...
                  <div style={{ fontFamily: 'var(--font-head)', fontWeight: 800, fontSize: '0.9rem', textTransform: 'uppercase', letterSpacing: '0.04em', color: 'var(--text)', lineHeight: 1.3, marginTop: 4 }}>{mod.title}</div>
                </div>
                <div className="fmp-sessions-grid">
                  {mod.sessions.map(s => (
                    <div key={s} style={{ display: 'flex', alignItems: 'flex-start', gap: 8 }}>
                      ...
                      <span style={{ fontSize: '0.82rem', color: 'var(--text-muted)', fontWeight: 300, lineHeight: 1.5 }}>{s}</span>
                    </div>
                  ))}
                </div>
              </motion.div>
          ...
          <div style={{ maxWidth: 720, margin: '0 auto' }}>
            ...
            <div className="fmp-form-card">
              <form onSubmit={handleSubmit(onSubmit)} style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
                ...
                <p style={{ fontSize: '0.75rem', color: 'var(--text-dim)', textAlign: 'center' }}>No spam. Your information is safe and will only be used to contact you about enrollment.</p>
              </form>
            </div>

            <div style={{ display: 'flex', justifyContent: 'center', gap: 32, marginTop: 24, flexWrap: 'wrap' }}>
              <a href="tel:6269956666" style={{ display: 'flex', alignItems: 'center', gap: 6, color: 'var(--text-muted)', fontSize: '0.85rem', fontWeight: 300, textDecoration: 'none' }}>
                <Phone size={14} color="var(--red)" /> 6269-956666
              </a>
              <a href="mailto:enquiry@arthashastra.business" style={{ display: 'flex', alignItems: 'center', gap: 6, color: 'var(--text-muted)', fontSize: '0.85rem', fontWeight: 300, textDecoration: 'none' }}>
                <Mail size={14} color="var(--red)" /> enquiry@arthashastra.business
              </a>
            </div>
          </div>
        </div>
      ...
      <style>{`
        .fmp-hero { min-height: 70vh; background: var(--bg); display: flex; align-items: center; padding-top: 100px; padding-bottom: 60px; position: relative; overflow: hidden; }
        .fmp-back-link { display: inline-flex; align-items: center; gap: 6px; color: var(--text-muted); font-family: var(--font-head); font-size: 0.75rem; letter-spacing: 0.12em; text-transform: uppercase; margin-bottom: 32px; transition: color 0.2s; }
        .fmp-h1 { font-family: var(--font-head); font-weight: 900; font-size: clamp(2.4rem, 5vw, 4.8rem); text-transform: uppercase; line-height: 1; color: var(--text); margin-bottom: 8px; }
        ...
        @media (max-width: 900px) {
          .fmp-hero-grid { grid-template-columns: 1fr !important; gap: 40px !important; }
          .fmp-module-row { grid-template-columns: 1fr !important; gap: 16px !important; }
          .fmp-sessions-grid { grid-template-columns: 1fr !important; }
          .fmp-why-grid { grid-template-columns: repeat(2, 1fr) !important; }
          .fmp-form-row { grid-template-columns: 1fr !important; }
        }

        @media (max-width: 600px) {
          .fmp-hero { padding-top: 80px; padding-bottom: 40px; }
          .fmp-stats-grid { grid-template-columns: repeat(2, 1fr) !important; }
          .fmp-module-row { padding: 20px 16px !important; }
          .fmp-why-grid { grid-template-columns: 1fr !important; }
          .fmp-form-card { padding: 24px 16px !important; }
        }
      `}</style>
    </div>
  )
